...
import { ReviewServiceError, type ReviewInput } from "@/services/review.service";
...
export const readJsonObject = async (request: Request): Promise<JsonObject> => {
  try {
    ...
    throw new ReviewServiceError("Request body must be valid JSON.", "INVALID_INPUT");
  }
};

export const parseMediaInput = (body: JsonObject): ReviewMediaInput => {
  ...
  if (
    ...
    throw new ReviewServiceError("Provide a valid movie or series.", "INVALID_INPUT");
  }
...
export const parseReviewInput = (body: JsonObject): ReviewInput => {
  return {
    ...
    title: typeof body.title === "string" || body.title === null ? body.title : null,
  };
};

export const reviewErrorResponse = (error: unknown) => {
