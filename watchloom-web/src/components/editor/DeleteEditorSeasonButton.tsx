...
type DeleteEditorSeasonButtonProps = {
  action: () => void | Promise<void>;
};

export function DeleteEditorSeasonButton({ action }: DeleteEditorSeasonButtonProps) {
  return (
    <form
      action={action}
      onSubmit={(event) => {
        if (!window.confirm("Delete this season and its episodes? This cannot be undone.")) {
